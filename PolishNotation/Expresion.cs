namespace PolishNotation
{
    ...
    public class Expression:IEnumerable<ElementaryUnit>
    {
        ...
        private static Expression ToExpresion(string expression)
        {
            ...
            for( var i = 0; i < expression.Length; i++)
            {
                ...
                if (char.IsLetter(c))
                {
                    ...
                    i = j - 1;
                    result.Add(new ElementaryUnit(Emums.ElementaryUnitType.Function, buffer));
                    continue;
                }
        ...
        private static Expression ToPolishNotation(Expression expression)
        {
            ...
            {
                if(el.Type == Emums.ElementaryUnitType.Digit || el.Type == Emums.ElementaryUnitType.Variable)
                {
                    result.Add(el);
                    continue;
                }
                ...
                if(el.Type == Emums.ElementaryUnitType.Brackets)
                {
                    if(el.Value == ")")
                    {
                        ...
                        if(buffer.Peek().Type == Emums.ElementaryUnitType.Function)
                        {
                            result.Add(buffer.Pop());
                        }
                        buffer.Pop();
                ...
                if(el.Type == Emums.ElementaryUnitType.BinaryOperation)
                {
                    if(el.Value == "^")
                    {
                        while (buffer.Count != 0 && (buffer.Peek().Value == "^" || buffer.Peek().Type == Emums.ElementaryUnitType.Function) )
                        {
                            result.Add(buffer.Pop());
                    ...
                    {
                        if(buffer.Count != 0)
                            while ((firstLO + secondLO).Contains(buffer.Peek().Value) || buffer.Peek().Value == "^" || buffer.Peek().Type == Emums.ElementaryUnitType.Function)
                            {
                                result.Add(buffer.Pop());
                                if (buffer.Count == 0) break;
                            }
                    ...
                    {
                        if (buffer.Count != 0)
                            while (buffer.Count != 0 && secondLO.Contains(buffer.Peek().Value) || buffer.Peek().Value == "^" && buffer.Count != 0 || buffer.Peek().Type == Emums.ElementaryUnitType.Function)
                            {
                                result.Add(buffer.Pop());
                                if (buffer.Count == 0) break;
                            }
